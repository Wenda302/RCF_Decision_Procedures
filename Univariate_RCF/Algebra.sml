...
fun p_pow (p : poly, n : int) =
    if n < 0 then raise Algebra_Error "Polynomial raised to negative power."
...
fun pp_div' (pp : pp, pp' : pp, result : pp) : pp =
    case (pp, pp') of
	([], []) => (List.rev result)
      | ([], _) => raise Algebra_Error "Unclean power-product division"
      | (pp, []) => (List.rev result) @ pp
      |  ((v, pow)::r, (v', pow')::r') =>
	 if v' > v then raise Algebra_Error "Unclean power-product division"
	 ...
	 else raise Algebra_Error "Unclean power-product division";
...
fun p_subst_neg_x p =
    case p of [] => []
	    ...
	    | _ => raise Algebra_Error "Only univariate polynomials supported."
...
fun mk_univ_in (p : poly, i : var_id) =
	...
	p_sum((poly_of_mono (c, [(i, p)])), mk_univ_in(ms, i))
      | _ => raise Algebra_Error ("mk_univ_in: Poly has pp of length > 1!"); 
...
fun p_integral p =
    let fun big_c p u =
		...
		[] => u
	      | ((c, _) :: ms) => 
		let val (_,b) = Rat.quotient_of_rat c
		in if b <> 1 then 
		       big_c ms (b*u)
		   else big_c ms u
		end
